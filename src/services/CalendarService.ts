...
export class CalendarService {
    ...
    static filterHostForGivenDay(team: Team, calendarEvent: CalendarEvent): TeamMember | null {
        ...
        let hostForDay = team.members.filter(x => x.lastHostDate.getDate() === calendarEvent.startTime.getDate());
    ...
    static filterTeamMemberToRoster(team: Team, calendarEvent: CalendarEvent, rosterCheck: RosterCheck): TeamMember | null {

        switch (rosterCheck) {
            case RosterCheck.Available: return CalendarService.filterAvailableTeamMember(team, calendarEvent);
            case RosterCheck.AcceptedInvite: return CalendarService.filterInviteAcceptedTeamMember(team, calendarEvent);
            case RosterCheck.AcceptedAndAvailable: return CalendarService.filterAcceptedAndAvailableTeamMember(team, calendarEvent);
            case RosterCheck.None: return team.members[0];
        }
    }

    private static filterInviteAcceptedTeamMember(team: Team, calendarEvent: CalendarEvent): TeamMember | null {

        let inviteAcceptedMembers = CalendarService.filterInviteAcceptedTeamMembers(team, calendarEvent);
    ...
    private static filterInviteAcceptedTeamMembers(team: Team, calendarEvent: CalendarEvent): TeamMember[] | null {
        ...
        let teamMembers = team.members.filter((member) => {
            return guests.some((guest) => {
                return guest.getEmail() == member.emailAddress &&
    ...
    private static filterAcceptedAndAvailableTeamMember(team: Team, calendarEvent: CalendarEvent): TeamMember | null {
    ...
    private static filterAvailableTeamMember(team: Team, calendarEvent: CalendarEvent): TeamMember | null {
    ...
    private static isMemberCalendarFree(teamMember: TeamMember, calendarEvent: CalendarEvent): boolean {
        ...
        if (calendar == null) {
            try {
                ...
                throw new CalendarNotAccessibleError(403, `Could not access ${emailAddress} calendar for team member`, teamMember);
            }
        }
    ...
    static getScheduledCalendarEvent(calendarEvent: CalendarEvent): GoogleAppsScript.Calendar.CalendarEvent | null {
